##so fixing so that everything is on the same plot

##stuff<- list(names2, files, scales.point, list.mats)
##names(stuff)<- c('names2', 'files', 'scales.point', 'list.mats')
##save(stuff, file='C:\\TopicDiscovery\\NecessaryParts.RData')

##alright, thinking about how to use the points
##we need the origin point, and then the next point

visualize<- function(circle=T, start=c(0,0), noclust=25){
 
z<- 0
orig.point<- start
store<- func.test(orig.point, noclust=noclust)
Sys.sleep(1)
if(circle==T){
